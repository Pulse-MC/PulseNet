...
public class Metrics {
   ...
   private static final ScheduledExecutorService metricsScheduler = Executors.newSingleThreadScheduledExecutor(r -> {
      Thread t = new Thread(r, "pulse-fabric-metrics");
      t.setDaemon(true);
      return t;
   });
   ...
   private static boolean isMetricsEnabled(){
      if(PulseFabric.CONFIG == null) return true;
      return PulseFabric.CONFIG.getBoolean(PulseFabric.METRICS_ENABLED);
   }
   
   private static int getUpdateInterval(){
      if(PulseFabric.CONFIG == null) return 1;
      int val = PulseFabric.CONFIG.getInt(PulseFabric.METRICS_UPDATE_INTERVAL);
      return Math.max(1, val);
   }
   
   private static boolean isModuleCpuEnabled(){
      if(PulseFabric.CONFIG == null) return true;
      return PulseFabric.CONFIG.getBoolean(PulseFabric.METRICS_MODULE_CPU);
   }
   
   private static boolean isModuleNetworkEnabled(){
      if(PulseFabric.CONFIG == null) return true;
      return PulseFabric.CONFIG.getBoolean(PulseFabric.METRICS_MODULE_NETWORK);
   }
   
   private static boolean isModuleMemoryEnabled(){
      if(PulseFabric.CONFIG == null) return true;
      return PulseFabric.CONFIG.getBoolean(PulseFabric.METRICS_MODULE_MEMORY);
   }
   
   private static boolean isModuleWriteQueueEnabled(){
      if(PulseFabric.CONFIG == null) return true;
      return PulseFabric.CONFIG.getBoolean(PulseFabric.METRICS_MODULE_WRITE_QUEUE);
   }
}
