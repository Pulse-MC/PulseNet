...
public class Metrics {
   ...
   private static final ScheduledExecutorService metricsScheduler = Executors.newSingleThreadScheduledExecutor(r -> {
      Thread t = new Thread(r, "pulsenet-metrics");
      t.setDaemon(true);
      return t;
   });
   ...
   private static boolean isMetricsEnabled(){
      if(PulseNet.CONFIG == null) return true;
      return PulseNet.CONFIG.getBoolean(PulseNet.METRICS_ENABLED);
   }
   
   private static int getUpdateInterval(){
      if(PulseNet.CONFIG == null) return 1;
      int val = PulseNet.CONFIG.getInt(PulseNet.METRICS_UPDATE_INTERVAL);
      return Math.max(1, val);
   }
   
   private static boolean isModuleCpuEnabled(){
      if(PulseNet.CONFIG == null) return true;
      return PulseNet.CONFIG.getBoolean(PulseNet.METRICS_MODULE_CPU);
   }
   
   private static boolean isModuleNetworkEnabled(){
      if(PulseNet.CONFIG == null) return true;
      return PulseNet.CONFIG.getBoolean(PulseNet.METRICS_MODULE_NETWORK);
   }
   
   private static boolean isModuleMemoryEnabled(){
      if(PulseNet.CONFIG == null) return true;
      return PulseNet.CONFIG.getBoolean(PulseNet.METRICS_MODULE_MEMORY);
   }
   
   private static boolean isModuleWriteQueueEnabled(){
      if(PulseNet.CONFIG == null) return true;
      return PulseNet.CONFIG.getBoolean(PulseNet.METRICS_MODULE_WRITE_QUEUE);
   }
}
