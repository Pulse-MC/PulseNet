...
public class ConfigManager {
   ...
   public void read(){
      try(BufferedReader input = new BufferedReader(new InputStreamReader(new FileInputStream(file)))){
         PulseNet.LOGGER.debug("Reading {} config...", modName);
         ...
         PulseNet.LOGGER.debug("Initialising {} config...", modName);
         this.values.forEach(value -> value.value = value.defaultValue);
      }catch(IOException e){
         PulseNet.LOGGER.fatal("Failed to load {} config file!", modName);
         PulseNet.LOGGER.fatal(e);
      }catch(Exception e){
         PulseNet.LOGGER.fatal("Failed to parse {} config", modName);
         PulseNet.LOGGER.fatal(e);
      }
   }
   
   public void save(){
      PulseNet.LOGGER.debug("Updating {} config...", modName);
      try(BufferedWriter output = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file)))){
         ...
         PulseNet.LOGGER.fatal("Failed to save {} config file!", modName);
         PulseNet.LOGGER.fatal(e);
      }
   }
   ...
   public Object getValue(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error(e.toString());
      }
      return null;
   }
   ...
   public int getInt(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error("Config {}:{} is not numeric for getInt (type={})", this.modId, setting.getName(), value != null ? value.getClass().getName() : "null");
      }catch(Exception e){
         PulseNet.LOGGER.error("Failed to get Integer config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return 0;
   }
   ...
   public float getFloat(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error("Config {}:{} is not numeric for getFloat (type={})", this.modId, setting.getName(), value != null ? value.getClass().getName() : "null");
      }catch(Exception e){
         PulseNet.LOGGER.error("Failed to get Float config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return 0;
   }
   ...
   public boolean getBoolean(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error("Failed to get Boolean config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return false;
   }
   ...
   public double getDouble(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error("Config {}:{} is not numeric for getDouble (type={})", this.modId, setting.getName(), value != null ? value.getClass().getName() : "null");
      }catch(Exception e){
         PulseNet.LOGGER.error("Failed to get Double config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return 0;
   }
   ...
   public <T> List<T> getList(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error("Config {}:{} is not a List for getList (type={})", this.modId, setting.getName(), value != null ? value.getClass().getName() : "null");
      }catch(Exception e){
         PulseNet.LOGGER.error("Failed to get List config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return List.of();
   }
   ...
   public List<String> getStringList(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error("Failed to get String List config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return List.of();
   }
   ...
   public List<Integer> getIntList(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error("Failed to get Integer List config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return List.of();
   }
   ...
   public List<Double> getDoubleList(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error("Failed to get Double List config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return List.of();
   }
   ...
   public List<Float> getFloatList(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error("Failed to get Float List config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return List.of();
   }
   ...
   public List<Boolean> getBooleanList(IConfigSetting<?> setting){
      try{
         ...
         PulseNet.LOGGER.error("Failed to get Boolean List config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return List.of();
   }
   ...
   public <E extends Enum<E>> List<E> getEnumList(IConfigSetting<?> setting, Class<E> enumClass){
      try{
         ...
         PulseNet.LOGGER.error("Failed to get Enum List config for {}:{}", this.modId, setting.getName());
         PulseNet.LOGGER.error(e.toString());
      }
      return List.of();
   }
